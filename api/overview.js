// api/overview.js  (CORRIGIDO)
// ─────────────────────────────────────────────────────────────
// Busca dados reais da visão geral de uma das 4 contas ML.
//
// CORREÇÃO APLICADA: a busca de pedidos não tinha `sort=date_desc`,
// então o ML devolvia os pedidos MAIS ANTIGOS primeiro. Resultado:
// - gráfico "Vendas por dia" sempre zerado
// - "Última venda" mostrava a venda mais antiga (ou nada)
// Agora os pedidos vêm dos mais recentes para os mais antigos.
// ─────────────────────────────────────────────────────────────
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ★ CORREÇÃO: sort=date_desc para receber os pedidos mais recentes primeiro
    const ordersBaseUrl = `https://api.mercadolibre.com/orders/search?seller=${mlUserId}&order.status=paid&sort=date_desc`;
    ...
    // Vendas por dia (últimos 14 dias) — agora funciona porque os
    // pedidos vêm ordenados do mais recente para o mais antigo
    ...
    // Última venda — com sort=date_desc, orders[0] é realmente a mais recente
...
// ★ CORREÇÃO: para de paginar se a API devolver erro (antes ignorava
// silenciosamente e o faturamento saía menor que o real).
async function fetchAllOrdersRevenue(mlUserId, accessToken, dateFrom, dateTo, firstPageData) {
  ...
  const baseUrl = `https://api.mercadolibre.com/orders/search?seller=${mlUserId}` +
    `&order.status=paid&sort=date_desc&order.date_created.from=${dateFrom}&order.date_created.to=${dateTo}&limit=${limit}`;
  ...
  for (let page = 1; page < pages; page++) {
    ...
    if (!res.ok) {
      // A API do ML limita o offset e a taxa de requisições.
      // Registra o problema em vez de devolver um número errado em silêncio.
      console.warn(`[overview] Paginação de pedidos parou na página ${page} (HTTP ${res.status}). Faturamento pode estar parcial.`);
      break;
    }
